...
3. Continue if it succeeds and returns that `account_slug`. If it fails, call `list_accounts`:
   - If a team there has the state file's `account_id` under a different slug, the team was renamed. Call `switch_account` with that team's current slug. If it succeeds, continue: use the new slug for every later switch in this session, but keep the state file's slug for local `.hamster/` paths. Tell the user once that this repository's Hamster state still has the old slug, and to run `hamster init --force --account-id <id>` here to refresh it.
   - In every other case, including when no team has that `account_id` (the MCP sign-in can't see this team, often a different user than the CLI) or the switch to the new slug fails, make no other Hamster MCP calls: tell the user the MCP team couldn't be set, quote the error, and stop.
