...
If this client has the Hamster MCP tools (hosted at `https://tryhamster.com/mcp`), point them at this repository's team before any Hamster MCP call, including `get_task` or `search`: if `.hamster/.state.json` exists at the repository root and the tools include `switch_account`, call it with the file's `account_slug`. If that fails or doesn't return the file's `account_id`, make no more Hamster MCP calls, don't fall back to `hamster chat`, and don't look the item up anywhere else (including the web): tell the user this repository's Hamster team isn't available to this MCP sign-in, and to run `hamster init --force` here or sign in to Hamster MCP as the right user, then stop. [mcp-account](references/mcp-account.md) has the details.
