...
Pi lists the skills as `/skill:<skill>`. Pi has no MCP support, so Hamster runs through the CLI: run `/skill:setup` to install it and sign in, or follow [Advanced: CLI binary](#advanced-cli-binary).
